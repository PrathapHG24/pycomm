...
def batch_read(plc_ip, tags):
    try:
        results = {}
        with ThreadPoolExecutor(max_workers=20) as executor:  # Adjust max_workers based on your system capabilities
...
def insert_data_to_plc():
    try:
        ...
        batch_size = 10  # Reducing the batch size for faster writes
        with ThreadPoolExecutor(max_workers=20) as executor:  # Increase max_workers based on your system capabilities
            futures = []
...
def read_data_from_plc():
    try:
        ...
        results = batch_read(plc_ip, tags_to_read)
